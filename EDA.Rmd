...
There are several interesting parts of this data and some questions we could be
exploring. As a first step we will be starting with the ratings.
...
- Ratings - some of them are rated TV-X and some have the typical ratings that
are associated with movies. Because of that I have split them by type of medium.
However, this is perhaps not ideal either, since some movies are made for
TV and thus would have the TV ratings. Additionally, we can see that there are 
some movies without a rating at all (the first bar). A second look shows that 
it is `r nrow(data_in[rating == ""])` without any rating, which is not that 
many rows overall and we could probably jsut drop them if we are trying to 
use the ratings column.
```{r, warning=FALSE}
