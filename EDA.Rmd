...
#### Checks
Data from the internet (and in real life) is often messy or dirty. While the 
columns of this one look relatively tidy and there are no duplicated columns or strangely named features of the shows, it is good to
...
There are several questions we can answer with this data. First we
...
of Asian heritage, judging from the names and some quick googling, with a heavy representation from Bollywood actors. I was a bit surprised to find that the two Japanese names on 
...
ggplot(lots_of_directors, aes(x = N, fill = type)) +
  geom_histogram() +
  facet_wrap(~type)
```
...
```{r}
lots_of_directors[, sum(perc_total), .(type)]
```
